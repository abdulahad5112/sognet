...
/// paths which are:
/// 1. `/opt/sognet` if it not present here then it fallbacks to the next one (2)
/// 2. Under project folder ( or codebase in other words) if it is not present
///    here then it returns an error as mentioned above.
pub fn handle_different_public_path() -> Result<String, Error> {
    if Path::new(format!("/opt/sognet/{}/", PUBLIC_DIRECTORY_NAME).as_str()).exists() {
        Ok(format!("/opt/sognet/{}", PUBLIC_DIRECTORY_NAME))
