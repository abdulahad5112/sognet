...
// ------- Constants --------
static COMMON_DIRECTORY_NAME: &str = "sognet";
...
impl Config {
    ...
    /// paths which are:
    /// 1. `~/.config/sognet/` if it not present here then it fallbacks to the next one (2)
    /// 2. `/etc/xdg/sognet/config.lua` if it is not present here then it fallbacks to the next
    ///    one (3).
    /// 3. `sognet/` (under project folder ( or codebase in other words)) if it is not present
